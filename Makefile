...
build-go:
	cd ./src;go build -o app-one service.go;cd ../

build-go-arm:
	cd ./src;GOOS=linux GOARCH=arm GOARM=6 go build -ldflags="-s -w" -o app-one service.go;cd ../

build-go-amd:
	cd ./src;GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o app-one service.go;cd ../
...
package-tar:
	tar cvzf app-one_$(version).tar.gz app-one VERSION
...
package-deb-doc:clean-deb
	...
	cp ./src/app-one package/debian/opt/thingsplex/app-one
	cp VERSION package/debian/opt/thingsplex/app-one
...
package-docker-amd:build-go-amd
	cp ./src/app-one package/docker/service
	cd ./package/docker;docker build -t app-one .

deb-arm : clean configure-arm build-go-arm package-deb-doc
	@echo "Building Thingsplex ARM package"
	mv package/debian.deb package/build/app-one_$(version)_armhf.deb

deb-amd : configure-amd64 build-go-amd package-deb-doc
	@echo "Building Thingsplex AMD package"
	mv package/debian.deb app-one_$(version)_amd64.deb

upload :
	@echo "Uploading the package to remote host"
	scp package/build/app-one_$(version)_armhf.deb $(remote_host):~/

remote-install : upload
	@echo "Uploading and installing the package on remote host"
	ssh -t $(remote_host) "sudo dpkg -i app-one_$(version)_armhf.deb"
